...
namespace FilteringUtility
{
    internal class Program
    {
        static async Task Main(string[] args)
        {
            ...
            {
                var dataFilePath = !Path.Exists(dataPath)
                ...
                if (!Path.Exists(dataFilePath))
                {
                    ...
                    return; // После регистрации поменять путь к логам не удастся (если только свой logger делать)
                }

                ConfigureServices(services, ref logsPath, dataFilePath);
                ...
                Console.WriteLine($"Путь к файлу с данными: {dataFilePath}");
